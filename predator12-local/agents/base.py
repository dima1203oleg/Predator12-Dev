...
class BaseAgent(ABC):
    ...
    async def _execute_plan(self, task: AgentTask, plan: ExecutionPlan) -> AgentResult:
        ...
        try:
            for i, step in enumerate(plan.steps):
                ...
                logger.info(
                    f"Agent {self.name} executing step {i+1}/{len(plan.steps)}: {step['name']}"
                )

                try:
                    # Execute step with timeout
                    step_result = await asyncio.wait_for(
                        self.execute_step(step, context), timeout=task.timeout_seconds
                    )
            ...
            return AgentResult(
                ...
                data={"executed_steps": executed_steps, "final_context": context},
        ...
        if plan.fallback_steps:
            try:
                ...
                for step in plan.fallback_steps:
                    ...
                    executed_steps.append(
                        {"step": step, "result": step_result, "success": True, "fallback": True}
                    )
                    context.update(step_result.get("context_updates", {}))
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "resource_usage": {"memory_mb": 0, "cpu_percent": 0},  # Would check actual usage
