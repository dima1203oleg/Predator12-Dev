...
class AutoHealAgent:
                ...
                if self.active_incidents:
                    ...
                    if plan:
                        ...
                        if success:
                            ...
                            self.logger.info(
                                f"Successfully remediated incident: {incident.description}"
                            )
                        else:
                            # Mark as failed and try later
                            incident.severity = (
                                "critical" if incident.severity != "critical" else "critical"
                            )
                            self.logger.error(
                                f"Failed to remediate incident: {incident.description}"
                            )
        ...
        try:
            async with self.postgres_pool.acquire() as conn:
                ...
                return {
                    ...
                    "max_query_duration": (
                        str(long_queries[0]["max_duration"]) if long_queries else None
                    ),
    ...
    async def _is_duplicate_incident(self, incident: Incident) -> bool:
        ...
        for active_incident in self.active_incidents:
            if (
                ...
                and active_incident.affected_component == incident.affected_component
            ):
                return True
        return False
    ...
    async def _execute_remediation_plan(self, plan: RemediationPlan) -> bool:
        ...
        current_success_rate = HEAL_ACTIONS_COUNTER._value.get() / max(
            1, INCIDENTS_DETECTED._value.get()
        )
    ...
    async def _cleanup_disk(self) -> bool:
        ...
        try:
            ...
            subprocess.run(
                ["find", "/var/log", "-name", "*.log", "-mtime", "+7", "-delete"], check=False
            )
