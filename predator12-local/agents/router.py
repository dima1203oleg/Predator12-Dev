...
class ModelSelector:
    ...
    def select_model(
        ...
        if not candidate_models:
            logger.warning(
                f"No models found with capabilities {required_capabilities}, using default"
            )
...
class AgentRouter:
    ...
    def route_task(self, task: AgentTask) -> Optional[str]:
        ...
        for agent_name, capabilities in self.agent_capabilities.items():
            score = self._calculate_agent_score(
                agent_name, task, capabilities, required_capabilities
            )
    ...
    def create_agent_instance(
        ...
        primary_model = self.model_selector.select_model(task, agent_type, required_capabilities)
        ...
        try:
            agent = agent_class(
                name=f"{agent_name}_{task.id[:8]}", agent_type=agent_type, config=agent_config
            )
    ...
    def _requires_review(self, task: AgentTask, agent_name: str) -> bool:
        ...
        if (
            task.priority in [TaskPriority.CRITICAL, TaskPriority.EMERGENCY]
            and task.risk_level == RiskLevel.HIGH
        ):
            return True

        # Certain agent types doing risky operations
        risky_operations = ["deploy", "migrate", "delete", "restart", "modify"]
        if any(op in task.description.lower() for op in risky_operations) and task.risk_level in [
            RiskLevel.MEDIUM,
            RiskLevel.HIGH,
        ]:
            return True
    ...
    def get_routing_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "agent_types": {
                agent_name: self._get_agent_type(agent_name) for agent_name in self.agents.keys()
            },
...
# Export main classes
__all__ = ["AgentRouter", "ModelSelector", "ModelMetadata", "ModelType"]
