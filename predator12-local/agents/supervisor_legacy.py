...
class Supervisor:
    ...
    def load(self):
        ...
        for agent, cfg in pol.items():
            ...
            self.policies[agent] = AgentPolicy(
                ...
                rate_limit_per_minute=int(
                    cfg.get("rate_limit_per_minute", defaults.get("rate_limit_per_minute", 30))
                ),
    ...
    def status(self):
        ...
        for name, cfg in self.registry.items():
            ...
            print(
                f" - {name}: primary={cfg.primary} fallbacks={cfg.fallbacks} embedding={cfg.embedding} policy={pol}"
            )
    ...
    def run_loop(self):
        ...
        try:
            ...
            while True:
                ...
                if self.dry_run:
                    print(
                        f"[SUP][HB#{hb}] tick: {len(self.registry)} agents monitored; self_improve={self.self_improve_enabled}"
                    )
                time.sleep(5)
...
def main():
    ...
    ap.add_argument(
        "--dry-run", action="store_true", help="Do not spawn real agents, print decisions only"
    )
    ap.add_argument(
        "--cmd",
        default="status",
