...
class SelfHealingAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = None, **config):
        ...
        self.resource_thresholds = config.get(
            "resource_thresholds",
    ...
    async def _check_system_resources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Системні ресурси: CPU {cpu_percent:.1f}%, RAM {memory_percent:.1f}%, Disk {disk_percent:.1f}%"
            )
    ...
    async def _check_services(self, services: List[str] = None) -> Dict[str, Any]:
        ...
        for service_url in services:
            try:
                ...
                service_status[service_url] = {"status": "unavailable", "error": str(e)}
        ...
        while True:
            try:
                ...
                if health_status in ["warning", "critical"]:
                    logger.warning(
                        f"Виявлено проблеми здоров'я: {summary.get('issues_found', 0)} issues"
                    )
...
# Several modules import specific self-heal agent classes. The implementation
# was consolidated into SelfHealingAgent; provide aliases so older imports
# continue to work during tests and local development.
BackupRestoreAgent = SelfHealingAgent
ConfigurationHealAgent = SelfHealingAgent
DataIntegrityAgent = SelfHealingAgent
DependencyFixAgent = SelfHealingAgent
MonitoringRepairAgent = SelfHealingAgent
NetworkHealAgent = SelfHealingAgent
PerformanceRecoveryAgent = SelfHealingAgent
SecurityPatchAgent = SelfHealingAgent
ServiceRestartAgent = SelfHealingAgent
SystemRecoveryAgent = SelfHealingAgent
