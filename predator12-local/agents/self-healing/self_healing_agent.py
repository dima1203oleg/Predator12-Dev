    ...
    async def check_service_health(self, service_name: str, endpoint: str) -> HealthCheck:
        ...
        try:
            ...
            async with session.get(endpoint, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                ...
                if resp.status == 200:
                    return HealthCheck(
                        service=service_name, status="healthy", response_time=response_time
                    )
                else:
                    # AI аналіз проблеми
                    context = {"urgency": "high", "complexity": 0.6, "problem_type": "http_error"}
                    ...
                    messages = [
                        {
                            "role": "system",
                            "content": "You are a system diagnostics expert. Analyze HTTP error and suggest action.",
                        },
                        {
                            "role": "user",
                            "content": f"Service {service_name} returned HTTP {resp.status}. Endpoint: {endpoint}. What action should be taken?",
                        },
                    ]
    ...
    async def analyze_system_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are a system performance analyst. Analyze metrics and identify issues.",
                },
                {
                    "role": "user",
                    "content": f"System metrics: {json.dumps(metrics, indent=2)}. Identify performance issues and suggest optimizations.",
                },
            ]
    ...
    async def plan_healing_action(self, health_checks: List[HealthCheck]) -> List[HealingAction]:
        ...
        try:
            ...
            problems_description = "\n".join(
                [f"- {hc.service}: {hc.status} ({hc.error_message})" for hc in unhealthy_services]
            )

            messages = [
                {
                    "role": "system",
                    "content": "You are an expert system administrator. Plan healing actions for unhealthy services. Respond with specific actionable steps.",
                },
                {
                    "role": "user",
                    "content": f"Unhealthy services:\n{problems_description}\n\nPlan healing actions with risk assessment.",
                },
            ]
    ...
    async def execute_healing_action(self, action: HealingAction) -> Dict[str, Any]:
        ...
        try:
            ...
            if action.action_type == "restart_service":
                ...
                result = {"status": "skipped", "reason": "Action type not implemented"}
    ...
    async def learn_from_healing(self, action: HealingAction, result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are a self-learning system. Analyze healing action results and extract lessons.",
                },
                {
                    "role": "user",
                    "content": f"Healing action: {action.action_type} on {action.target_service}. Result: {result}. What can we learn?",
                },
            ]
    ...
    async def run_healing_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cycle_result = {
                ...
                "overall_health": (
                    "healthy" if all(hc.status == "healthy" for hc in health_checks) else "degraded"
                ),
