...
class SelfDiagnosisAgent:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            for service_name, url in endpoints:
                try:
                    ...
                    services[service_name] = {"status": "unreachable", "error": str(e)}
    ...
    async def _update_metric_history(self, metrics: Dict[str, Any]):
        ...
        for metric in key_metrics:
            if metric in metrics:
                ...
                # Remove old entries
                self.metric_history[metric] = [
                    (ts, val) for ts, val in self.metric_history[metric] if ts > cutoff_time
                ]
    ...
    async def _calculate_health_score(
        self, diagnoses: List[DiagnosisResult], metrics: Dict[str, Any]
    ) -> float:
    ...
    async def _process_prometheus_alert(self, alert: Dict[str, Any]):
        ...
        diagnosis = DiagnosisResult(
            ...
            severity=(
                ...
                if severity.lower() in [s.value for s in Severity]
