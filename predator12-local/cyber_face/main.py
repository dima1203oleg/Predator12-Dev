...
from .vision import AttentionTracker, EmotionDetector, FaceDetector
...
class CyberFaceApplication:
    ...
    async def process_video_frame(self, frame_data: bytes) -> dict:
        ...
        try:
            ...
            expression = self.avatar_animator.expression_mapper.map_emotion_to_expression(
                emotion_result
            )
        ...
        try:
            ...
            if not self.current_session:
                self.current_session = ConversationContext(
                    user_id="voice_user", session_id="voice_session", conversation_history=[]
                )
            ...
            response = await self.conversation_engine.generate_response(
                transcript, self.current_session
            )
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "avatar_performance": (
                self.avatar_renderer.get_performance_stats() if self.avatar_renderer else {}
            ),
        }
