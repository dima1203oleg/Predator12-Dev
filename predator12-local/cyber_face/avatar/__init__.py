...
class ExpressionMapper:
    ...
    def __init__(self):
        self.expression_mappings = {
            ...
            "happy": ExpressionBlendShapes(
                eyebrows_outer_up=0.3, mouth_smile=0.8, eye_squint_left=0.2, eye_squint_right=0.2
            ),
            "sad": ExpressionBlendShapes(
                eyebrows_inner_up=0.6, mouth_frown=0.7, eye_blink_left=0.3, eye_blink_right=0.3
            ),
            "angry": ExpressionBlendShapes(
                eyebrows_down=0.8, mouth_frown=0.5, eye_squint_left=0.4, eye_squint_right=0.4
            ),
            "surprised": ExpressionBlendShapes(
                eyebrows_inner_up=0.9, eyebrows_outer_up=0.9, mouth_open=0.6, jaw_open=0.4
            ),
            "fearful": ExpressionBlendShapes(
                eyebrows_inner_up=0.8, eye_blink_left=0.1, eye_blink_right=0.1, mouth_open=0.3
            ),
            "disgusted": ExpressionBlendShapes(eyebrows_down=0.4, mouth_frown=0.6, cheek_puff=0.3),
            "focused": ExpressionBlendShapes(
                eyebrows_down=0.2, eye_squint_left=0.1, eye_squint_right=0.1
            ),
            "confused": ExpressionBlendShapes(
                eyebrows_inner_up=0.4, eyebrows_down=0.2, mouth_pucker=0.3
            ),
            "stressed": ExpressionBlendShapes(
                eyebrows_inner_up=0.5, eyebrows_down=0.3, mouth_frown=0.4
            ),
        }
    ...
    def map_emotion_to_expression(self, emotion: EmotionResult) -> ExpressionType:
        ...
        return emotion_to_expression.get(emotion.primary_emotion, ExpressionType.NEUTRAL)

    def get_blend_shapes(
        self, expression: ExpressionType, intensity: float = 1.0
    ) -> ExpressionBlendShapes:
        """Get blend shape weights for expression"""
        base_shapes = self.expression_mappings.get(
            expression.value, self.expression_mappings["neutral"]
        )
...
class AvatarAnimator:
    ...
    async def update_expression(
        self, expression: ExpressionType, intensity: float = 1.0, duration: float = None
    ):
    ...
    async def update_eye_direction(self, direction: Tuple[float, float], duration: float = None):
    ...
    async def _animate_head_pose(self, animation: Dict, progress: float):
        ...
        new_position = tuple(
            current_pos[i] + (target_position[i] - current_pos[i]) * progress for i in range(3)
        )

        # Interpolate rotation
        current_rot = self.current_state.head_rotation
        new_rotation = tuple(
            current_rot[i] + (target_rotation[i] - current_rot[i]) * progress for i in range(3)
        )
...
class AvatarRenderer:
    ...
    async def _setup_lighting(self):
        ...
        # Fill light
        fill_light = {"type": "AmbientLight", "color": "#404040", "intensity": 0.3}
    ...
    async def _setup_animations(self):
        ...
        self.animation_mixer = {"type": "AnimationMixer", "clips": [], "actions": {}}
    ...
    async def update_avatar(self, state: AvatarState):
        ...
        try:
            ...
            blend_shapes = self.expression_mixer.get_blend_shapes(
                state.expression, state.expression_intensity
            )
