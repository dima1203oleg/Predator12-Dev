...
class IntentClassifier:
    ...
    def __init__(self):
        self.intent_patterns = {
            ...
            "farewell": ["goodbye", "bye", "see you", "talk later", "exit"],
        }
...
class ConversationEngine:
    ...
    def __init__(
        self, config, openai_key: Optional[str] = None, anthropic_key: Optional[str] = None
    ):
        self.config = config
    ...
    async def _select_model(
        self, user_input: str, intent: Intent, context: ConversationContext
    ) -> ModelProvider:
    ...
    async def _generate_actions(
        ...
        if intent.name == "query_system_status":
            ...
            actions.append(
                {"type": "navigate", "target": "/dashboard/agents", "label": "View Agent Status"}
            )

        elif intent.name == "request_analysis":
            actions.append({"type": "modal", "target": "analytics", "label": "Open Analytics"})
