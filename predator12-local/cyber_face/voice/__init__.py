...
class SpeechRecognition:
    ...
    async def _init_cloud_providers(self):
        ...
        if google_credentials:
            try:
                self.providers[VoiceProvider.GOOGLE] = GoogleSpeechProvider(
                    google_credentials, self.config
                )
            except Exception as e:
    ...
    async def recognize_once(
        self, language: str = "en-US", timeout: float = 5.0
        ...
        try:
            ...
            result = await asyncio.wait_for(provider.recognize_once(language), timeout=timeout)
    ...
    async def _handle_speech_result(self, result: SpeechResult):
        ...
        if result.confidence >= self.confidence_threshold:
            logger.debug(
                f"Speech recognized: {result.transcript} (confidence: {result.confidence})"
            )
            ...
            logger.debug(
                f"Low confidence speech ignored: {result.transcript} (confidence: {result.confidence})"
            )
...
class VoiceCommandProcessor:
    ...
    def __init__(self):
        self.command_patterns = {
            "show_dashboard": ["show dashboard", "open dashboard", "main screen"],
            "check_agents": ["check agents", "agent status", "how are agents"],
            "system_health": ["system health", "health check", "status report"],
            "open_analytics": ["open analytics", "show analytics", "data analysis"],
            "run_diagnosis": ["run diagnosis", "diagnose", "check problems"],
...
class WebSpeechProvider:
    ...
    async def start_listening(self, **kwargs):
        pass

    async def recognize_once(self, language):
        pass
