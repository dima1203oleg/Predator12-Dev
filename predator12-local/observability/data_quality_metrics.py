...
DQ_RECORDS_PROCESSED = Counter(
    "dq_records_processed_total", "Total records processed", ["source", "status"]
)

DQ_VALIDATION_ERRORS = Counter(
    "dq_validation_errors_total", "Data validation failures", ["type", "severity"]
)

DQ_FIELD_COMPLETENESS = Gauge(
    "dq_field_completeness_ratio", "Field completeness percentage", ["field"]
)
...
class DataQualityTracker:
    ...
    def record_processed(self, count: int, status: str = "success"):
        ...
        DQ_RECORDS_PROCESSED.labels(source=self.source, status=status).inc(count)

    def record_validation_error(self, error_type: str, severity: str = "warning"):
        """Log validation issues"""
        DQ_VALIDATION_ERRORS.labels(type=error_type, severity=severity).inc()

    def measure_completeness(self, field: str, present: int, total: int):
        """Track field completeness"""
        DQ_FIELD_COMPLETENESS.labels(field=field).set(present / total)
