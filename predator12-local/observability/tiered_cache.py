...
class TieredCache:
    ...
    def __init__(
        self, redis_client: redis.Redis, memory_ttl=60, redis_ttl=300, max_memory_items: int = 1000
    ):
    ...
    def cached(self, func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if redis_data:
                ...
                self.memory_cache[key] = {"value": value, "expires": now + self.memory_ttl}
                self.memory_cache.move_to_end(key)
                # Enforce LRU limit
            ...
            self.memory_cache[key] = {"value": result, "expires": now + self.memory_ttl}
            self.memory_cache.move_to_end(key)
            if len(self.memory_cache) > self.max_memory_items:
                self.memory_cache.popitem(last=False)
                evict_ctr.inc()
