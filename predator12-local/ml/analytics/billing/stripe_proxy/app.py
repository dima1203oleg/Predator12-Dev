...
async def create_checkout_session(request: CreateCheckoutSessionRequest, db=Depends(get_db)):
    """Create a Stripe Checkout Session for subscription or one-time payment"""
    try:
        # Check if user already has a Stripe customer ID
        subscription = (
            db.query(Subscription).filter(Subscription.user_id == request.user_id).first()
        )
        ...
        if not customer_id:
            ...
            customer = stripe.Customer.create(metadata={"user_id": str(request.user_id)})
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            metadata={"user_id": str(request.user_id), **(request.metadata or {})},
        )
...
async def create_customer_portal(request: CreateCustomerPortalRequest, db=Depends(get_db)):
...
async def get_subscription_status(user_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        if not subscription:
            return SubscriptionStatus(user_id=user_id, active=False)
...
@app.post("/webhook")
async def stripe_webhook(request: Request, background_tasks: BackgroundTasks, db=Depends(get_db)):
