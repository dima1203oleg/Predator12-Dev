...
class RedTeamAgent:
    ...
    def __init__(self, environment, model="llama3", attack_frequency=24):
        ...
        logger.info(
            f"RedTeamAgent initialized with model {model} and attack frequency {attack_frequency}h"
        )
    ...
    async def run_background(self):
        ...
        while True:
            try:
                ...
                if self.last_attack_time is None or (
                ...
                ) > timedelta(hours=self.attack_frequency):
                    ...
                    if vulnerabilities:
                        logger.warning(
                            f"Security test found {len(vulnerabilities)} vulnerabilities"
                        )
