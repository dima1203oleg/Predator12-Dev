...
def analyze_import_operations(company_ids=None, time_period=None):
    ...
    """
    logger.info(
        f"Analyzing import operations for {len(company_ids) if company_ids else 'all'} companies"
    )
    ...
    try:
        ...
        logger.info(
            f"Import operations analysis completed with {len(all_anomalies)} anomalies detected"
        )
        return results
                        ...
                        anomalies.append(
                            {
                                ...
                                "declaration_date": (
                                    row["declaration_date"].isoformat()
                                    if isinstance(row["declaration_date"], datetime)
                                    else row["declaration_date"]
                                ),
            ...
            for product_desc, desc_group in group.groupby("product_description"):
                if len(desc_group["product_code"].unique()) > 1:
                    ...
                    for code in product_codes:
                        ...
                        avg_tax_rate = (
                            code_group["tax_amount"].sum() / code_group["customs_value"].sum() * 100
                        )
                            ...
                            anomalies.append(
                                {
                                    ...
                                    "tax_rate_difference": float(
                                        tax_rates[-1][1] - tax_rates[0][1]
                                    ),
                                    "risk_score": min(
                                        100, float((tax_rates[-1][1] - tax_rates[0][1]) * 5)
                                    ),
            ...
            anomalies.append(
                {
                    ...
                    "company_names": [
                        df[df["company_id"] == c]["company_name"].iloc[0] for c in sample_companies
                    ],
                    "risk_score": 75.0,
