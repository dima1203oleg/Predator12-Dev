...
with engine.connect() as conn:
    results = conn.execute(
        text("SELECT id, product_description, importer_name, exporter_name FROM customs_registry")
    ).fetchall()
...
for row in results:
    combined_text = (
        f"{row.product_description or ''} {row.importer_name or ''} {row.exporter_name or ''}"
    )
