...
class CSVHandler(FileSystemEventHandler):
    def on_created(self, event):
        ...
        if event.src_path.endswith(".csv"):
            ...
            update_job(
                filename, status="pending", started=None, finished=None, rows=None, error=None
            )
            ...
            with open(log_path, "w") as logf:
                try:
                    ...
                    r1 = subprocess.run(
                        ["python3", "scripts/csv_to_db.py"], capture_output=True, text=True
                    )
                    logf.write(
                        "[csv_to_db.py stdout]\n"
                        + r1.stdout
                    ...
                    r2 = subprocess.run(
                        ["python3", "scripts/postgres_to_qdrant.py"], capture_output=True, text=True
                    )
                    logf.write(
                    ...
                    update_job(
                        filename, status="done", finished=datetime.now().isoformat(), error=None
                    )
                except Exception as e:
                    print(f"[Watcher] Помилка при виконанні ETL: {e}")
                    update_job(
                        filename, status="error", finished=datetime.now().isoformat(), error=str(e)
                    )
