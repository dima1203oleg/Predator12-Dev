...
from ..services.social_media_service import SocialMediaService, social_media_service
...
@router.get("/social-media-influencers", response_model=InfluencerAnalyticsResponse)
async def get_influencers(
    platform: Optional[str] = Query(
        None, description="Filter by social media platform (e.g., Twitter, Telegram)"
    ),
    min_followers: int = Query(0, description="Minimum number of followers"),
    topic: Optional[str] = Query(None, description="Filter by a key topic of influence"),
...
@router.post("/influencer-impact", response_model=List[InfluencerImpactResponse])
