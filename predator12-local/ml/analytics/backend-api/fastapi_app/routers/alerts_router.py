...
from ..auto_remediation import RemediationAction, RemediationError, remediate
from ..dependencies import get_settings
from ..logging import log_alert_to_database, logger
...
async def handle_alert(alert: Alert, settings: Annotated[Settings, Depends(get_settings)]):
    ...
    if (
        ...
        try:
            ...
            logger.info(
                f"Auto-remediation action for alert '{alert.name}': {action_taken.action_type} - {action_taken.details}"
            )
            ...
            logger.error(
                f"An unexpected error occurred during auto-remediation for alert '{alert.name}': {e}"
            )
