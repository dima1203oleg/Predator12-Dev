...
g_forecast_risk = Gauge(
    "mas_forecast_risk", "MAS forecasted risk (1=high, 0=normal)", ["agent", "metric"]
)
    ...
    while True:
        ...
        for metric_name, prom_metric, threshold in metrics:
            ...
            for item in results:
                ...
                if risk:
                    print(
                        f"[Forecast] High risk for {agent} on {metric_name} (avg={np.mean(values[-5:]):.2f})"
                    )
        time.sleep(300)
