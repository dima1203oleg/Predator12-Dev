...
async def restart_agent(agent_id, namespace=None):  # Changed to async def
    ...
    if not KUBERNETES_ASYNCIO_AVAILABLE:
        logging.error("kubernetes_asyncio library is not available. " "Cannot restart agent.")
        return
    ...
    try:
        ...
        async with k8s_client.ApiClient() as api_client:
            ...
            # Changed to await
            pods = await v1.list_namespaced_pod(namespace=ns, label_selector=label_selector)
            if not pods.items:
                logging.warning(f"No pod found for agent {agent_id} in namespace {ns}")
                return
            for pod in pods.items:
                pod_name = pod.metadata.name
                # Changed to await
                await v1.delete_namespaced_pod(name=pod_name, namespace=ns)
                logging.info(f"Restarted pod {pod_name} for agent {agent_id} " f"in namespace {ns}")
...
async def notify_alertmanager(agent_id, metric):  # Changed to async def
    ...
    if not alertmanager_url:
        ...
        return
    alert = {
        "labels": {"alertname": "MASAnomaly", "agent": agent_id, "metric": metric},
        "annotations": {"summary": f"Anomaly detected for {agent_id} on {metric}"},
    }
    try:
        async with aiohttp.ClientSession() as session:  # Use aiohttp
            # Changed to async with
            async with session.post(f"{alertmanager_url}/api/v1/alerts", json=[alert]) as response:
                # Raise an exception for HTTP errors
                response.raise_for_status()
                logging.info(
                    f"Alert sent for agent {agent_id} on {metric}. " f"Status: {response.status}"
                )
