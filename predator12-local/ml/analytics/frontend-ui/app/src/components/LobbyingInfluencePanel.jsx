...
const LobbyingInfluencePanel = () => {
    ...
    const handleAnalyze = async () => {
        ...
        setError(null);
        try {
            const response = await fetch('/api/analyze-lobbying', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="p-6 bg-gray-900 text-white rounded-lg shadow-lg max-w-md mx-auto">
            <h2 className="text-2xl font-bold mb-4">Lobbying Influence Analysis</h2>
            <textarea 
                ...
                placeholder="Enter your query for lobbying influence analysis..."
                rows={4}
                disabled={isLoading}
            />
            <button 
                ...
                disabled={isLoading}
            >
                {isLoading ? 'Analyzing...' : 'Analyze Lobbying'}
            </button>
            {error && (
            ...
            {analysisResult && !error && (
                <div className="mt-4 p-4 bg-gray-800 rounded-md">
                    <h3 className="text-lg font-semibold mb-2">Lobbying Analysis Result:</h3>
...
export default LobbyingInfluencePanel; 