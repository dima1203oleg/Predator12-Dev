...
def auto_train_model_task(
    self, model_name: str, dataset_id: str, parameters: Dict[str, Any]
) -> Dict[str, Any]:
...
def data_quality_analysis_task(self, dataset_id: str, rules: List[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        overall_score = sum(check["score"] for check in quality_checks.values()) / len(
            quality_checks
        )
        ...
        logger.info(
            f"Data quality analysis completed for dataset {dataset_id}, score: {overall_score}"
        )
        return result
