...
def calculate_simulation_metrics(
    results_data: List[Dict], config: ScenarioConfig
) -> Dict[str, float]:
    ...
    if config.model_type == "monte_carlo":
        ...
        metrics = {
            ...
            "network_stability": sum(r["network_density"] for r in results_data)
            / len(results_data),
            "emergence_score": sum(r["emergent_properties"]["clustering"] for r in results_data)
            / len(results_data),
        }
        ...
        metrics = {
            ...
            - abs(sum(r["feedback_loops"] for r in results_data) / len(results_data)),
    ...
    visualizations.append(
        {
            ...
            "data": {
                ...
                "y": [
                    r.get("value", r.get("average_behavior", r.get("queue_length", 0)))
                    for r in results_data
                ],
            },
            "config": {"x_label": "Time", "y_label": "Value", "color": "#00FFC6"},
        }
    )
...
async def cancel_simulation(simulation_id: str):
    ...
    if sim_data["status"] in ["completed", "failed", "cancelled"]:
        raise HTTPException(
            status_code=400,
            detail=f"Cannot cancel simulation with status: {sim_data['status']}",
        )
...
@router.get("/list")
async def list_simulations(status: Optional[str] = None, limit: int = 50) -> Dict[str, Any]:
    """
        ...
        ScenarioConfig(
            ...
            parameters=[
                SimulationParameter(
                    ...
                    type="number",
                    value=1,
                    description="Number of initially compromised nodes",
                ),
                SimulationParameter(
                    name="infection_rate",
                    type="range",
                    value=[0.1, 0.3],
                    description="Rate of infection spread",
                ),
                SimulationParameter(
                    name="detection_probability",
                    type="number",
                    value=0.7,
                    description="Probability of detecting infected node",
                ),
                SimulationParameter(
                    name="mitigation_effectiveness",
                    type="range",
                    value=[0.5, 0.9],
                    description="Effectiveness of mitigation measures",
                ),
            ],
            duration=100.0,
        ...
        ScenarioConfig(
            ...
            parameters=[
                SimulationParameter(
                    ...
                    type="number",
                    value=1000,
                    description="Initial resource pool",
                ),
                SimulationParameter(
                    name="demand_variability",
                    type="range",
                    value=[0.8, 1.2],
                    description="Demand variation factor",
                ),
                SimulationParameter(
                ...
                SimulationParameter(
                    ...
                    type="number",
                    value=0.85,
                    description="Target efficiency level",
                ),
            ],
            duration=200.0,
        ...
        ScenarioConfig(
            ...
            parameters=[
                ...
                SimulationParameter(
                    ...
                    type="range",
                    value=[0.15, 0.25],
                    description="Market volatility range",
                ),
                SimulationParameter(
        ...
        ScenarioConfig(
            ...
            parameters=[
                SimulationParameter(
                    ...
                    type="range",
                    value=[5, 15],
                    description="Customer arrival rate per hour",
                ),
                SimulationParameter(
                    name="service_rate",
                    type="range",
                    value=[8, 12],
                    description="Service rate per hour",
                ),
                SimulationParameter(
                    name="queue_capacity",
                    type="number",
                ...
                SimulationParameter(
                    ...
                    type="boolean",
                    value=True,
                    description="Enable priority service",
                ),
            ],
