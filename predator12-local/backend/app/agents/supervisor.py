...
class AgentSupervisor:
    ...
    async def register_agent(self, agent_id: str, agent_obj: object) -> None:
        """Register an agent instance with the supervisor.

        Tests expect that calling register_agent will insert the named
        agent into `self.agents` mapping and make it retrievable.
        """
        self.agents[agent_id] = agent_obj

    async def execute_command(self, agent_id: str, command: str, payload: dict) -> dict:
        """Execute a command on a registered agent.

        This is a minimal implementation used by tests. If the agent
        exposes an async `submit_task` method (BaseAgent pattern), we
        await it and return a simple task summary. If the agent is not
        found or doesn't support command execution, raise ValueError.
        """
        if agent_id not in self.agents:
        ...
        # Prefer async submit_task if available
        submit = getattr(agent, "submit_task", None)
        if submit and asyncio.iscoroutinefunction(submit):
            task_id = await submit(command, payload)
        elif submit:
            # fallback to sync call
            task_id = submit(command, payload)
        else:
            raise ValueError(f"Agent {agent_id} cannot execute commands")
    ...
    def _run_self_healing_agent(self):
        ...
        try:
            ...
            if health_report.get("overall_status") == "critical":
                ...
                self.logger.info(
                    f"Auto-healing completed: {len(healing_report.get('actions_taken', []))} actions taken"
                )
    ...
    def _run_model_monitor_agent(self):
        ...
        try:
            ...
            if retraining_needed:
                self.logger.info(
                    f"Models scheduled for retraining: {[m['model_name'] for m in retraining_needed]}"
                )
