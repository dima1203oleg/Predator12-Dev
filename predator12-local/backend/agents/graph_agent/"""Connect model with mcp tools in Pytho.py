            ...
            if response.choices[0].message.tool_calls:
                for tool in response.choices[0].message.tool_calls:
                    ...
                    if tool_name in self._tool_to_server_map:
                        ...
                        messages.append(
                            ToolMessage(tool_call_id=tool.id, content=str(result.content))
                        )
            else:
...
async def main():
    ...
    messages = [
        SystemMessage(
            content="You are a web exploration assistant that can navigate to websites, take screenshots, and provide summaries of web content. You'll help users explore the web by visiting sites and extracting useful information."
        ),
        UserMessage(
            content=[
                TextContentItem(
                    text="Please navigate to https://www.example.com and perform the following tasks:\n1. Take a screenshot of the main page\n2. Extract the main heading and purpose of the website\n3. Provide a brief summary of what the website is all about (2-3 sentences)"
                ),
            ]
        ),
    ]
