...
exports.AgentControlCenter = void 0;
const react_1 = __importStar(require("react"));
const AgentControlCenter = ({ agents }) => {
    ...
    const [isTerminalOpen, setIsTerminalOpen] = (0, react_1.useState)(false);
    const [terminalInput, setTerminalInput] = (0, react_1.useState)('');
    // Симуляция выполнения команд
    (0, react_1.useEffect)(() => {
        const interval = setInterval(() => {
            setCommands(prev => prev.map(cmd => {
                if (cmd.status === 'pending') {
                    return Object.assign(Object.assign({}, cmd), { status: 'executing' });
                }
                else if (cmd.status === 'executing' && Math.random() > 0.7) {
                    const result = generateCommandResult(cmd.command);
                    const completedCommand = Object.assign(Object.assign({}, cmd), { status: 'completed', result });
    ...
    const generateCommandResult = (command) => {
        const results = {
        ...
        return commandType ? results[commandType] : 'Command executed successfully';
    };
    const executeCommand = (agentId, command, parameters = {}) => {
        const newCommand = {
        ...
        <div>
          {selectedAgentData ? (<>
              <div style={{
                ...
                {selectedAgentData.specialty && (<p style={{ color: '#ccc', fontSize: '14px', lineHeight: '1.4' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px', marginBottom: '20px' }}>
                {quickCommands.map((cmd) => (<button key={cmd.command} onClick={() => executeCommand(selectedAgent, cmd.command)} style={{
              ...
              <div style={{
                ...
                alignItems: 'center',
                  ...
                  <button onClick={() => setIsTerminalOpen(!isTerminalOpen)} style={{
                background: 'none',
                border: 'none',
                color: '#fff',
                cursor: 'pointer',
                ...
                {isTerminalOpen && (<div style={{ padding: '16px' }}>
                    <div style={{
                    maxHeight: '200px',
                    overflowY: 'auto',
                    marginBottom: '12px',
                    ...
                    var _a;
                    return (<div key={cmd.id} style={{ marginBottom: '8px' }}>
                          <div style={{ color: '#888' }}>
                            [{new Date(cmd.timestamp).toLocaleTimeString()}]
                            {(_a = agents.find(a => a.id === cmd.agentId)) === null || _a === void 0 ? void 0 : _a.name}
                          </div>
                          <div style={{ color: '#8B5CF6' }}>
                            $ {cmd.command}
                          </div>
                          {cmd.result && (<div style={{ color: '#10B981', marginLeft: '8px' }}>
                              {cmd.result}
                            </div>)}
                        </div>);
                })}
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <input type="text" value={terminalInput} onChange={(e) => setTerminalInput(e.target.value)} onKeyPress={(e) => {
                    if (e.key === 'Enter' && terminalInput.trim()) {
                        executeCommand(selectedAgent, terminalInput);
                        setTerminalInput('');
                    }
                }} placeholder="Enter custom command..." style={{
                    flex: 1,
                    background: 'rgba(255, 255, 255, 0.1)',
                    border: '1px solid rgba(255, 255, 255, 0.2)',
                    borderRadius: '4px',
      ...
      {commands.length > 0 && (<div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '150px', overflowY: 'auto' }}>
            {commands.filter(c => c.status !== 'completed').map((cmd) => {
                ...
                <div>
                  <div style={{ color: '#fff', fontSize: '14px', fontWeight: '600' }}>
                    {(_a = agents.find(a => a.id === cmd.agentId)) === null || _a === void 0 ? void 0 : _a.name} • {cmd.command}
                  </div>
...
exports.AgentControlCenter = AgentControlCenter;
exports.default = exports.AgentControlCenter;
