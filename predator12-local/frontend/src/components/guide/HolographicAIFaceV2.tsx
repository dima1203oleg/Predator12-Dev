...
interface HolographicAIFaceProps {
  isActive: boolean;
  isSpeaking: boolean;
  emotion?: 'neutral' | 'alert' | 'processing' | 'success' | 'error';
  message?: string;
  intensity?: number;
  ...
  enableSoundWaves?: boolean;
  enableEnergyRings?: boolean;
  systemHealth?: 'optimal' | 'warning' | 'critical';
  cpuLoad?: number;
  memoryUsage?: number;
  autoPosition?: boolean;
  fixedPosition?: { top?: number; bottom?: number; left?: number; right?: number };
  collisionAvoidance?: boolean;
  onCollision?: (collisionElements: Element[]) => void;
...
const CanvasFallbackFace: React.FC<{
  emotion: string;
  isSpeaking: boolean;
  intensity: number;
  systemHealth: string;
  size: string;
}> = ({ emotion, isSpeaking, intensity, systemHealth, size }) => {
  ...
  const config = fallbackSizeConfig[size as keyof typeof fallbackSizeConfig] || fallbackSizeConfig.small;
...
export const HolographicAIFace: React.FC<div/* HolographicAIFaceProps> = ({
  isActive = true,
  isSpeaking = false,
  emotion = 'neutral',
  message = '',
  ...
  const dataStreamRef = useRef<THREE.Points | null>(null);
  const alertSystemRef = useRef<THREE.Points | null>(null);
  ...
  useEffect(() => {
    if (!isActive || fallbackMode || forceCanvasFallback) return;
  ...
  useEffect(() => {
    if (shouldUseFallback || !isActive) return;
  ...
  if (shouldUseFallback) {
    return (
      ...
      >
        ...
        {/* System indicators for Canvas mode */}
        {(cpuLoad > 0.8 || memoryUsage > 0.8) && (
          <Box
            sx={{
              position: 'absolute',
              top: -10,
