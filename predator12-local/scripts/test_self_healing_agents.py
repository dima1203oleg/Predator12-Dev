        ...
        try:
            ...
            for container in containers:
                try:
                    ...
                    logger.error(f"Помилка при аналізі контейнера {container.name}: {exc}")
        except Exception as exc:
            logger.error(f"Помилка підключення до Docker: {exc}")
    ...
    async def heal_container(self, container_name: str, action: str) -> bool:
        ...
        try:
            ...
            if action == "restart":
                ...
                container.restart()
                return True
            if action == "health_check_restart":
            ...
            logger.error(f"❌ Помилка при лікуванні {container_name}: {exc}")
            return False
        return False
...
async def main():
    ...
    for container in containers:
        status_icon = (
            "✅" if container["status"] == "running" and not container["needs_healing"] else "❌"
        )
        health_icon = "🟢" if container["health"] in ["healthy", "no-health-check"] else "🔴"
        print(
            f"{status_icon} {health_icon} {container['name']:<30} | {container['status']:<10} | {container['health']}"
        )
