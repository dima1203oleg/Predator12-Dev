...
class RealModelTester:
    def __init__(self):
        ...
        self.results = {
            ...
            "servers": SERVERS,
            "results": {},
            "summary": {"working": 0, "failed": 0, "unavailable": 0, "errors": []},
        }
        ...
        try:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    logging.warning(
                        f"⚠️ {model} на {server['name']}: {response.status} - {error_text[:100]}"
                    )
    ...
    def save_results(self):
        ...
        working_models = [
            model for model, result in self.results["results"].items() if result["working"]
        ]
...
async def create_optimized_distribution(working_models):
    ...
    for i, agent_name in enumerate(agent_names):
        ...
        optimized_config["agents"][agent_name] = {
            "primary_models": (
                agent_models[: len(agent_models) // 2] if len(agent_models) > 1 else agent_models
            ),
            "fallback_models": (
                agent_models[len(agent_models) // 2 :] if len(agent_models) > 1 else []
            ),
            "max_concurrent": 3,
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    optimized_path = (
        f"/Users/dima/Documents/Predator11/OPTIMIZED_WORKING_MODELS_DISTRIBUTION_{timestamp}.json"
    )
