...
class ValuesValidator:
        ...
        if autoscale.get("enabled"):
            ...
            if max_replicas > 20:
                self.warnings.append(
                    f"maxReplicas={max_replicas} is very high. " "Consider cost implications."
                )
    ...
    def check_resources(self, values: Dict):
        ...
        if limits.get("memory"):
            ...
            if isinstance(mem, str) and "Gi" in mem:
                ...
                if gb > 16:
                    self.warnings.append(f"Memory limit {mem} is very high")

        # Check CPU limits
        if limits.get("cpu"):
            cpu = limits["cpu"]
            if isinstance(cpu, (int, float)) and cpu > 8:
                self.warnings.append(f"CPU limit {cpu} cores is very high")
    ...
    def check_features(self, values: Dict):
        ...
        for feature in dangerous:
            if features.get(feature, {}).get("enabled"):
                self.warnings.append(
                    f"Feature '{feature}' enabled - " "ensure proper testing before production"
                )
        ...
        if features.get("selfHealing", {}).get("enabled"):
            ...
            if not obs.get("prometheus", {}).get("enabled"):
                self.errors.append("selfHealing requires observability.prometheus.enabled=true")
    ...
    def check_security(self, values: Dict):
        ...
        if not security.get("vault", {}).get("enabled"):
            self.warnings.append("Vault is disabled - secrets management may be insecure")

        if not security.get("rbac", {}).get("enabled"):
            self.errors.append("RBAC must be enabled for production")

        if not security.get("networkPolicy", {}).get("enabled"):
            self.warnings.append("NetworkPolicy is disabled - no network segmentation")
    ...
    def check_observability(self, values: Dict):
        ...
        if not obs.get("otel", {}).get("enabled"):
            self.warnings.append("OpenTelemetry is disabled - no distributed tracing")

        if not obs.get("prometheus", {}).get("enabled"):
            self.warnings.append("Prometheus is disabled - no metrics collection")
