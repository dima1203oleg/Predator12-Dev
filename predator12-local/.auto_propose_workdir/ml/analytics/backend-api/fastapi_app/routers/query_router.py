...
from dependencies import get_current_user, get_db_session, get_redis_client
...
async def process_user_query(
    ...
    try:
        ...
        cache_key = generate_cache_key(
            query_data.query_text, query_data.context or {}, user.get("id", "anonymous")
        )
        ...
        await redis_client.setex(name=cache_key, time=3600, value=response.json())  # 1 hour TTL
        ...
        logger.error(f"Error processing query for user " f"{user.get('id', 'anonymous')}: {str(e)}")
        raise HTTPException(
...
def generate_cache_key(query_text: str, context: Dict[str, Any], user_id: str) -> str:
    """
...
async def store_query_history(
    db_session, user_id: str, query_text: str, query_plan: Dict[str, Any], results: Dict[str, Any]
):
    """
    ...
    try:
        ...
        logger.error(f"Failed to store query history for user {user_id}: " f"{str(e)}")
