...
from ..models.social_media_models import RegionSentiment, SentimentUpdateMessage
...
MOCK_TOPICS = ["economy", "politics", "technology", "healthcare", "environment"]
...
async def generate_mock_sentiment_update() -> SentimentUpdateMessage:
    ...
    return SentimentUpdateMessage(
        timestamp=datetime.now(timezone.utc).isoformat(), region_sentiments=region_sentiments
    )
...
async def websocket_health_check():
    ...
    """
    return {"status": "healthy", "message": "WebSocket service is operational."}
