...
class SocialMediaService:
    def __init__(self):
        ...
        self.mock_influencers = [
            ...
            InfluencerProfile(
                ...
                key_topics=["privacy", "messaging", "crypto"],
                verified=True,
            ),
            InfluencerProfile(
                id="tw_vitalik",
            ...
            InfluencerProfile(
                ...
                key_topics=["space", "science", "exploration"],
                verified=True,
            ),
            InfluencerProfile(
                id="yt_mkbhd",
                ...
                key_topics=["tech", "reviews", "smartphones"],
                verified=True,
            ),
        ]

    async def get_social_media_influencers(
        self, platform: Optional[str] = None, min_followers: int = 0, topic: Optional[str] = None
    ) -> InfluencerAnalyticsResponse:
        """
        ...
        if platform:
            filtered_influencers = [
                inf for inf in filtered_influencers if inf.platform.lower() == platform.lower()
            ]

        if min_followers > 0:
            filtered_influencers = [
                inf for inf in filtered_influencers if inf.followers_count >= min_followers
            ]
        ...
        platform_summary = {}
        for inf in filtered_influencers:
            platform_summary[inf.platform] = platform_summary.get(inf.platform, 0) + 1

        return InfluencerAnalyticsResponse(
            influencers=filtered_influencers, platform_summary=platform_summary
        )
    ...
    async def analyze_influencer_impact(
        ...
        for influencer_id in impact_request.influencer_ids:
            influencer = next(
                (inf for inf in self.mock_influencers if inf.id == influencer_id), None
            )
            if influencer:
                ...
                if region_id:
                    ...
                    if influencer_id.startswith("tw_") and region_id.startswith("USA_"):
                        # Higher impact in USA for Twitter influencers
                        region_modifier = 1.5
                    elif influencer_id.startswith("tg_") and region_id.startswith("EU_"):
                ...
                responses.append(
                    InfluencerImpactResponse(
                        ...
                        estimated_reach=int(  # Mock reach
                            influencer.followers_count * len(keywords) // 2 * region_modifier
                        ),
                        ...
                        key_themes_in_posts=[
                            f"theme_{kw}_{random.randint(1,3)}" for kw in keywords
                        ],
