...
def scrape_tender_data(source="prozorro", limit=100):
    ...
    try:
        ...
        logger.info(
            f"Completed tender data scraping from {source}: {results['tenders_scraped']} tenders"
        )
        return results
...
def scrape_company_data(source="edr", company_ids=None, limit=100):
    ...
    try:
        ...
        logger.info(
            f"Completed company data scraping from {source}: {results.get('companies_scraped', 0)} companies"
        )
        return results
...
def scrape_telegram_data(channels=None, limit_per_channel=100):
    ...
    try:
        ...
        logger.info(
            f"Completed Telegram data scraping: {results['total_messages']} messages from {results['channels_scraped']} channels"
        )
        return results
...
def parse_pdf_documents(document_ids=None, document_type="tender"):
    ...
    """
    logger.info(
        f"Starting PDF parsing for {len(document_ids) if document_ids else 0} documents of type {document_type}"
    )
    ...
    try:
        ...
        logger.info(
            f"Completed PDF parsing: {results['successful_parses']} successful, {results['failed_parses']} failed"
        )
        return results
...
def transform_data(data_type, data_ids=None, transformations=None):
    ...
    try:
        ...
        logger.info(
            f"Completed data transformation for {data_type}: {results['records_processed']} records"
        )
        return results
...
def cleanup_old_data(data_types=None, older_than_days=90):
    ...
    try:
        ...
        logger.info(
            f"Completed data cleanup: {sum(results['records_deleted'].values())} records deleted, {results['space_freed_mb']:.2f} MB freed"
        )
        return results
