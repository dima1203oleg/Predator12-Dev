...
async def ingest_csv(file: UploadFile = File(...)):
    ...
    try:
        ...
        for r in rows:
            meta = {
                "index": {
                    "_index": "customs_restricted_current",
                    "_id": r.get("doc_id"),
                }
            }
        ...
        if resp.status_code >= 300:
            ...
            raise HTTPException(
                status_code=502,
                detail=f"OpenSearch bulk failed: {resp.status_code} {snippet}",
            )
        ...
        if resp2.status_code >= 300:
            ...
            raise HTTPException(
                status_code=502,
                detail=f"OpenSearch bulk (masked) failed: {resp2.status_code} {snippet2}",
            )
