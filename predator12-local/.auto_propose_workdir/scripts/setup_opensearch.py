            ...
            for dashboard in dashboards.get("objects", []):
                if dashboard.get("type") == "dashboard":
                    ...
                    try:
                        ...
                        self.post(
                            f"_dashboards/api/saved_objects/dashboard/{dashboard_id}", dashboard
                        )
                        print(
                            f"✅ Imported dashboard: {dashboard.get('attributes', {}).get('title')}"
                        )
                    except Exception as e:
...
def setup_opensearch():
    ...
    try:
        ...
        print(
            f"🔍 Connected to OpenSearch cluster: {health.get('cluster_name')} (status: {health.get('status')})"
        )
    except Exception as e:
    ...
    data_streams = ["logs-predator-default", "metrics-predator-default", "traces-predator-default"]
