...
class PostgresToOpenSearchIndexer:
    ...
    async def create_indices_and_aliases(self):
        ...
        for table_name, table_config in self.config["tables"].items():
            ...
            await self.opensearch_client.indices.put_alias(index=safe_index, name=safe_alias)
        ...
        async with self.pg_pool.acquire() as conn:
            ...
            while offset < total_rows:
                ...
                for row in rows:
                    ...
                    safe_docs.append(
                        {"_index": safe_alias, "_id": row_dict["doc_id"], "_source": safe_doc}
                    )

                    # Створюємо обмежений документ (з повними даними)
                    restricted_docs.append(
                        {"_index": restricted_alias, "_id": row_dict["doc_id"], "_source": row_dict}
                    )
...
async def main():
    ...
    parser.add_argument("--cleanup-only", action="store_true", help="Only cleanup old indices")
