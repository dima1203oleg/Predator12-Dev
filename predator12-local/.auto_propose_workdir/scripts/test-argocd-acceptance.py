...
class ArgoCDAcceptanceTests:
    ...
    def test_argocd_cm(self) -> Tuple[bool, str]:
        success, output = run_command(f"kubectl get configmap argocd-cm -n {self.namespace}")
        return success, output if not success else ""

    def test_rbac_cm(self) -> Tuple[bool, str]:
        success, output = run_command(f"kubectl get configmap argocd-rbac-cm -n {self.namespace}")
        return success, output if not success else ""
    ...
    def test_app_project(self) -> Tuple[bool, str]:
        success, output = run_command(f"kubectl get appproject predator12 -n {self.namespace}")
        return success, output if not success else ""
    ...
    def test_dev_applications(self) -> Tuple[bool, str]:
        success, output = run_command(f"kubectl get application -n {self.namespace} -l env=dev")
    ...
    def test_staging_applications(self) -> Tuple[bool, str]:
        success, output = run_command(f"kubectl get application -n {self.namespace} -l env=staging")
    ...
    def test_prod_applications(self) -> Tuple[bool, str]:
        success, output = run_command(f"kubectl get application -n {self.namespace} -l env=prod")
    ...
    def test_argo_rollouts_installed(self) -> Tuple[bool, str]:
        success, output = run_command("kubectl get namespace argo-rollouts")
        return success, output if not success else ""

    def test_rollout_crds(self) -> Tuple[bool, str]:
        success, output = run_command("kubectl get crd rollouts.argoproj.io")
        return success, output if not success else ""

    def test_analysis_templates(self) -> Tuple[bool, str]:
        # Check if analysis template files exist
        success, output = run_command("ls infra/argo-rollouts/analysis-templates.yaml")
        return success, output if not success else ""

    def test_canary_services(self) -> Tuple[bool, str]:
        # Check if canary service manifests exist
        success, output = run_command("ls infra/argo-rollouts/services-ingress.yaml")
        return success, output if not success else ""

    def test_sealed_secrets(self) -> Tuple[bool, str]:
        success, output = run_command("kubectl get crd sealedsecrets.bitnami.com")
        return success, output if not success else ""

    def test_gatekeeper_installed(self) -> Tuple[bool, str]:
        success, output = run_command("kubectl get namespace gatekeeper-system")
        return success, output if not success else ""

    def test_gatekeeper_policies(self) -> Tuple[bool, str]:
        # Check if policy files exist
        success, output = run_command("ls infra/policy/gatekeeper-policies.yaml")
        return success, output if not success else ""
    ...
    def test_servicemonitors(self) -> Tuple[bool, str]:
        ...
        success, output = run_command("ls infra/argocd/base/servicemonitor.yaml")
        return success, output if not success else ""

    def test_prometheusrules(self) -> Tuple[bool, str]:
        # Check if prometheusrule files exist
        success, output = run_command("ls infra/argocd/base/prometheusrule.yaml")
        return success, output if not success else ""

    def test_metrics_endpoint(self) -> Tuple[bool, str]:
        # Check if metrics service exists
        success, output = run_command(f"kubectl get service argocd-metrics -n {self.namespace}")
        return success, output if not success else ""

    def test_presync_hooks(self) -> Tuple[bool, str]:
        success, output = run_command("ls infra/argocd/hooks/presync-db-migrate.yaml")
        return success, output if not success else ""

    def test_postsync_hooks(self) -> Tuple[bool, str]:
        success, output = run_command("ls infra/argocd/hooks/postsync-tests.yaml")
        return success, output if not success else ""

    def test_syncfail_hooks(self) -> Tuple[bool, str]:
        success, output = run_command("ls infra/argocd/hooks/syncfail-cleanup.yaml")
        return success, output if not success else ""
    ...
    def print_summary(self):
        ...
        if passed == total:
            print(
                f"{Colors.GREEN}{Colors.BOLD}🎉 All tests passed! ArgoCD GitOps stack is ready for production.{Colors.END}\n"
            )
        else:
            print(
                f"{Colors.RED}{Colors.BOLD}❌ Some tests failed. Please review and fix issues before deploying to production.{Colors.END}\n"
            )
