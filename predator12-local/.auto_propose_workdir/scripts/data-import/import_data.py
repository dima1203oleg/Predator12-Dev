    ...
    async def connect_databases(self):
        ...
        try:
            ...
            self.qdrant_client = QdrantClient(
                host=os.getenv("QDRANT_HOST", "localhost"), port=os.getenv("QDRANT_PORT", 6333)
            )
        ...
        for i in range(num_companies):
            company = {
                ...
                "status": np.random.choice(
                    ["active", "suspended", "liquidated"], p=[0.8, 0.15, 0.05]
                ),
                ...
                anomalies.append(
                    {
                        ...
                        "anomaly_type": np.random.choice(
                            ["financial_irregularity", "unusual_activity", "data_inconsistency"]
                        ),
...
async def main():
    ...
    parser.add_argument(
        "--companies", type=int, default=1000, help="Number of companies to generate"
    )
