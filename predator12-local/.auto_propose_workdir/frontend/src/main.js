...
const EnhancedComponents_1 = require("./components/EnhancedComponents");
const AIComponents_1 = require("./components/AIComponents");
const AIAgentsSection_1 = require("./components/ai/AIAgentsSection");
...
const MetricCard = ({ title, value, unit, icon, color, trend }) => {
    return (<div style={{
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        {/* Trend */}
        {trend !== undefined && (<div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px' }}>
...
const App = () => {
    ...
    const [models] = (0, react_1.useState)([
        {
            id: '1',
            name: 'GPT-4 Turbo',
            type: 'llm',
    ...
    const [activities] = (0, react_1.useState)([
        { agent: 'Alpha Agent', action: 'Completed data analysis task', time: '2 min ago', status: 'success' },
        { agent: 'Epsilon Coordinator', action: 'Coordinated 5 agents successfully', time: '5 min ago', status: 'success' },
        { agent: 'Delta Analyzer', action: 'Started training on new dataset', time: '8 min ago', status: 'info' },
        { agent: 'Beta Supervisor', action: 'Reviewed 23 tasks', time: '12 min ago', status: 'success' },
        { agent: 'Gamma Specialist', action: 'Failed to process image batch', time: '15 min ago', status: 'error' },
        { agent: 'Alpha Agent', action: 'Generated report successfully', time: '18 min ago', status: 'success' },
    ]);
    ...
    const [chartData] = (0, react_1.useState)(Array.from({ length: 20 }, (_, i) => ({
        time: `${i}:00`,
    ...
    return (<>
      ...
      <div style={{ position: 'relative', zIndex: 1, minHeight: '100vh', padding: '40px 20px' }}>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', marginBottom: '24px' }}>
          <EnhancedComponents_1.SearchBar value={searchQuery} onChange={setSearchQuery}/>

          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginTop: '16px' }}>
            <EnhancedComponents_1.FilterChip label="All Services" active={activeFilter === 'all'} onClick={() => setActiveFilter('all')} count={categoryCounts.all}/>
            <EnhancedComponents_1.FilterChip label="Core" active={activeFilter === 'core'} onClick={() => setActiveFilter('core')} count={categoryCounts.core}/>
            <EnhancedComponents_1.FilterChip label="Database" active={activeFilter === 'database'} onClick={() => setActiveFilter('database')} count={categoryCounts.database}/>
            <EnhancedComponents_1.FilterChip label="Search" active={activeFilter === 'search'} onClick={() => setActiveFilter('search')} count={categoryCounts.search}/>
            <EnhancedComponents_1.FilterChip label="AI/ML" active={activeFilter === 'ai'} onClick={() => setActiveFilter('ai')} count={categoryCounts.ai}/>
            <EnhancedComponents_1.FilterChip label="Monitoring" active={activeFilter === 'monitoring'} onClick={() => setActiveFilter('monitoring')} count={categoryCounts.monitoring}/>
            <EnhancedComponents_1.FilterChip label="Security" active={activeFilter === 'security'} onClick={() => setActiveFilter('security')} count={categoryCounts.security}/>
          </div>
        </div>
        ...
        <div style={{
          ...
          <MetricCard title="CPU Usage" value={metrics.cpu} unit="%" icon="⚡" color="#8B5CF6" trend={-2.3}/>
          <MetricCard title="Memory" value={metrics.memory} unit="%" icon="💾" color="#EC4899" trend={1.5}/>
          <MetricCard title="Disk" value={metrics.disk} unit="%" icon="💿" color="#3B82F6" trend={0.8}/>
          <MetricCard title="Network" value={metrics.network} unit="MB/s" icon="🌐" color="#10B981" trend={5.2}/>
        </div>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto 40px' }}>
          ...
          <AIComponents_1.AIStatsSummary totalAgents={agents.length} activeAgents={agents.filter(a => a.status === 'active').length} totalModels={models.length} totalRequests={models.reduce((sum, m) => sum + m.requests, 0)}/>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px', marginBottom: '40px' }}>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
                {agents.map((agent) => (<AIComponents_1.AgentCard key={agent.id} agent={agent}/>))}
              </div>
            </div>

            {/* Activity Timeline */}
            <div>
              <AIComponents_1.AgentActivityTimeline activities={activities}/>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
              {models.map((model) => (<AIComponents_1.ModelCard key={model.id} model={model}/>))}
            </div>
          </div>
        </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {activeFilter === 'all' || activeFilter === 'core' ? (<>
                  ...
                  {filteredServices.filter(s => s.category === 'core').map((service, i) => (<ServiceCard key={i} service={service} onClick={() => setSelectedService(service)}/>))}
                </>) : null}
              ...
              {activeFilter === 'all' || activeFilter === 'database' ? (<>
                  ...
                  {filteredServices.filter(s => s.category === 'database').map((service, i) => (<ServiceCard key={i} service={service} onClick={() => setSelectedService(service)}/>))}
                </>) : null}
              ...
              {activeFilter === 'all' || activeFilter === 'search' ? (<>
                  ...
                  {filteredServices.filter(s => s.category === 'search').map((service, i) => (<ServiceCard key={i} service={service} onClick={() => setSelectedService(service)}/>))}
                </>) : null}
              ...
              {activeFilter === 'all' || activeFilter === 'queue' ? (<>
                  ...
                  {filteredServices.filter(s => s.category === 'queue').map((service, i) => (<ServiceCard key={i} service={service} onClick={() => setSelectedService(service)}/>))}
                </>) : null}
              ...
              {activeFilter === 'all' || activeFilter === 'ai' ? (<>
                  ...
                  {filteredServices.filter(s => s.category === 'ai').map((service, i) => (<ServiceCard key={i} service={service} onClick={() => setSelectedService(service)}/>))}
                </>) : null}
              ...
              {activeFilter === 'all' || activeFilter === 'monitoring' ? (<>
                  ...
                  {filteredServices.filter(s => s.category === 'monitoring').map((service, i) => (<ServiceCard key={i} service={service} onClick={() => setSelectedService(service)}/>))}
                </>) : null}
              ...
              {activeFilter === 'all' || activeFilter === 'system' ? (<>
                  ...
                  {filteredServices.filter(s => s.category === 'system').map((service, i) => (<ServiceCard key={i} service={service} onClick={() => setSelectedService(service)}/>))}
                </>) : null}
              ...
              {activeFilter === 'all' || activeFilter === 'security' ? (<>
                  ...
                  {filteredServices.filter(s => s.category === 'security').map((service, i) => (<ServiceCard key={i} service={service} onClick={() => setSelectedService(service)}/>))}
                </>) : null}

              {filteredServices.length === 0 && (<div style={{
                textAlign: 'center',
                padding: '40px',
                color: '#888',
            }}>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          <AIAgentsSection_1.AIAgentsSection />
        </div>
      ...
      {/* Alert Notifications */}
      {alerts.map((alert) => (<EnhancedComponents_1.AlertNotification key={alert.id} alert={alert} onClose={() => setAlerts(alerts.filter((a) => a.id !== alert.id))}/>))}

      {/* Service Details Modal */}
      <EnhancedComponents_1.ServiceModal service={selectedService} onClose={() => setSelectedService(null)}/>
    </>);
};
// Mount
