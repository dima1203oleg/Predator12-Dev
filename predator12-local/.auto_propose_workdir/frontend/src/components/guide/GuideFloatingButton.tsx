...
interface GuideFloatingButtonProps {
  module: string;
...
const GuideFloatingButton: React.FC<GuideFloatingButtonProps> = ({
  module,
  ...
  return (
    <>
      <Box sx={getPositionStyles()}>
        ...
        >
          <Tooltip
            title={`AI Гід • ${module.toUpperCase()}`}
            placement="left"
            arrow
          >
            <Badge
              ...
              color="error"
              invisible={!hasNotifications || notificationCount === 0}
            >
              <Fab
                onClick={showGuide}
                sx={{
        ...
        <AnimatePresence>
          {isHovered && (
            ...
            >
              {/* Кнопка настроек */}
              <Tooltip title="Налаштування" placement="left">
                <Fab
                  size="small"
              ...
              {/* Кнопка звука */}
              <Tooltip
                title={settings.voice.synthesis ? "Вимкнути звук" : "Увімкнути звук"}
                placement="left"
              >
                <Fab
                  size="small"
      ...
      {/* Настройки гида */}
      <GuideSettingsManager
        open={showSettings}
        onClose={() => setShowSettings(false)}
        settings={settings}
        onSettingsChange={() => {}} // Will be handled by context
