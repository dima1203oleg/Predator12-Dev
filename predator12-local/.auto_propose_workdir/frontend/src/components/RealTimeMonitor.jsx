...
const RealTimeMonitor = () => {
    ...
    const [modelStatuses, setModelStatuses] = (0, react_1.useState)([
    ...
    (0, react_1.useEffect)(() => {
        const interval = setInterval(() => {
            ...
            setModelStatuses(prev => prev.map(model => (Object.assign(Object.assign({}, model), { load: Math.max(0, Math.min(100, model.load + Math.random() * 20 - 10)), responseTime: Math.max(50, model.responseTime + Math.random() * 200 - 100), errorRate: Math.max(0, model.errorRate + Math.random() * 1 - 0.5), status: model.status === 'recovering' && Math.random() > 0.7 ? 'active' :
    ...
    (0, react_1.useEffect)(() => {
        ...
        metricKeys.forEach((key, index) => {
            ...
            metrics.forEach((metric, i) => {
                ...
                const y = canvas.height - (metric[key] / 100) * canvas.height;
                if (i === 0) {
                    ctx.moveTo(x, y);
                }
                else {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px' }}>
        ...
        <div>
          ...
          <div style={{ position: 'relative', height: '300px', background: 'rgba(0,0,0,0.3)', borderRadius: '12px', overflow: 'hidden' }}>
            ...
            return (<div key={label} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{
                    width: '12px',
                    height: '12px',
                    borderRadius: '50%',
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {modelStatuses.map((model) => (<div key={model.id} style={{
                background: 'rgba(255, 255, 255, 0.05)',
...
exports.default = exports.RealTimeMonitor;
