...
const AdvancedAISystem_1 = __importDefault(require("../components/modules/AdvancedAISystem"));
const Agents = () => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        const interval = setInterval(() => {
            setAgents(prev => prev.map(agent => (Object.assign(Object.assign({}, agent), { efficiency: Math.max(70, Math.min(100, agent.efficiency + Math.floor(Math.random() * 6) - 3)), intelligence: Math.max(70, Math.min(100, agent.intelligence + Math.floor(Math.random() * 4) - 2)) }))));
        }, 5000);
    ...
    const getStatusColor = (status) => {
        switch (status) {
            ...
            case 'processing': return nexusTheme_1.nexusColors.sapphire;
            case 'learning': return nexusTheme_1.nexusColors.quantum;
    ...
    const getTypeIcon = (type) => {
        switch (type) {
            ...
            case 'predictor': return <icons_material_1.SmartToy />;
        }
    };
    if (view3D && (0, features_1.isFeatureEnabled)('threeDee')) {
        return (<material_1.Box sx={{ position: 'relative', height: '100vh', overflow: 'hidden' }}>
        <AdvancedAISystem_1.default agents={agents} onAgentInteraction={(agentId, action) => {
    ...
    return (<material_1.Box sx={{ p: 3, minHeight: '100vh' }}>
      ...
      <framer_motion_1.motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        <material_1.Box sx={{ mb: 4, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <material_1.Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <material_1.Chip label={`${agents.filter(a => a.status === 'active').length}/${agents.length} АКТИВНІ`} sx={{
          ...
          {(0, features_1.isFeatureEnabled)('threeDee') && (<material_1.Tooltip title="3D Візуалізація Агентів">
      ...
      <material_1.Grid container spacing={3}>
        <framer_motion_1.AnimatePresence>
          {agents.map((agent, index) => (<material_1.Grid item xs={12} md={6} lg={4} key={agent.id}>
              <framer_motion_1.motion.div initial={{ opacity: 0, scale: 0.8, rotateY: -15 }} animate={{ opacity: 1, scale: 1, rotateY: 0 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.6, delay: index * 0.1 }} whileHover={{ scale: 1.02, y: -8 }}>
                <material_1.Card sx={{
                ...
                textTransform: 'uppercase'
                    ...
                    {/* Current Task */}
                    {agent.currentTask && (<material_1.Box sx={{ mb: 2, p: 2, borderRadius: 2, background: `${nexusTheme_1.nexusColors.quantum}10` }}>
                        <material_1.Typography variant="caption" sx={{ color: nexusTheme_1.nexusColors.quantum, fontWeight: 600 }}>
                          CURRENT TASK
                        </material_1.Typography>
                    ...
                    <material_1.Box sx={{ mb: 2 }}>
                      {[
                { label: 'Інтелект', labelEn: 'Intelligence', value: agent.intelligence, color: nexusTheme_1.nexusColors.sapphire },
                { label: 'Ефективність', labelEn: 'Efficiency', value: agent.efficiency, color: nexusTheme_1.nexusColors.emerald },
                { label: 'Надійність', labelEn: 'Reliability', value: agent.reliability, color: nexusTheme_1.nexusColors.success }
            ].map(metric => (<material_1.Box key={metric.label} sx={{ mb: 1.5 }}>
                          <material_1.Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                            <material_1.Typography variant="caption" sx={{ color: nexusTheme_1.nexusColors.shadow }}>
                              {metric.label}
                            </material_1.Typography>
                            <material_1.Typography variant="caption" sx={{ color: metric.color, fontWeight: 600 }}>
                              {metric.value}%
                            </material_1.Typography>
                          </material_1.Box>
                          <material_1.LinearProgress variant="determinate" value={metric.value} sx={{
                    height: 6,
                    borderRadius: 3,
                    ...
                    <material_1.Box sx={{ mb: 2 }}>
                      ...
                      <material_1.Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                        {agent.capabilities.slice(0, 3).map(capability => (<material_1.Chip key={capability} label={capability} size="small" sx={{
                    backgroundColor: `${nexusTheme_1.nexusColors.emerald}15`,
                    ...
                    <material_1.Box sx={{ display: 'flex', gap: 1, pt: 2, borderTop: `1px solid ${nexusTheme_1.nexusColors.shadow}30` }}>
                      <material_1.Tooltip title="Пауза/Відновити">
                        <material_1.IconButton size="small" sx={{ color: nexusTheme_1.nexusColors.frost, '&:hover': { color: getStatusColor(agent.status) } }}>
                          {agent.status === 'active' ? <icons_material_1.Pause /> : <icons_material_1.PlayArrow />}
                        </material_1.IconButton>
                      </material_1.Tooltip>
