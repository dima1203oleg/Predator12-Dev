...
class OptimizationAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = None, **config):
        ...
        self.optimization_thresholds = config.get(
    ...
    async def execute_task(self, task: AgentTask, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, step in enumerate(plan["steps"]):
                ...
                logger.info(
                    f"Виконання кроку оптимізації {i+1}/{len(plan['steps'])}: {step['description']}"
                )
            ...
            optimization_metrics["improvements"] = await self._calculate_improvements(
                optimization_metrics["before"], optimization_metrics["after"]
            )
    ...
    async def _execute_optimization_step(
        ...
        try:
            if action == "collect_baseline_metrics":
                ...
                return {"success": False, "error": f"Невідома дія оптимізації: {action}"}
    ...
    async def _analyze_system_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Помилка аналізу продуктивності: {e}"}
        ...
        try:
            ...
            try:
                ...
                optimizations_applied.append(
                    f"Очищено Python garbage collection: {collected} об'єктів"
                )
            except Exception as e:
            ...
            try:
                if hasattr(psutil, "virtual_memory"):
                    ...
                    result = subprocess.run(
                        ["sudo", "sync"], capture_output=True, text=True, timeout=10
                    )
            ...
            return {"success": False, "error": f"Помилка оптимізації пам'яті: {e}"}
        ...
        try:
            ...
            if "cpu" in before and "cpu" in after:
                ...
                improvements["cpu"] = {
                    ...
                    "improvement_percent": (
                        (cpu_improvement / cpu_before * 100) if cpu_before > 0 else 0
                    ),
                }
            ...
            improvements["overall"] = {
                "average_improvement_percent": (
                    total_improvement / improvement_count if improvement_count > 0 else 0
                ),
            ...
            return {"error": f"Помилка розрахунку покращень: {e}", "before": before, "after": after}
    ...
    async def get_optimization_recommendations(self, target: str = "system") -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Помилка генерації рекомендацій: {e}"}
