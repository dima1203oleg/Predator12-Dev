...
class DataQualityAgent:
        ...
        async def run_validation(request: dict):
            ...
            try:
                ...
                result = await self.validate_dataset(
                    dataset_id=dataset_id, index_name=index_name, custom_rules=rules
                )
                return result
    ...
    async def validate_dataset(
        ...
        logger.info(
            "Starting data quality validation", dataset_id=dataset_id, records_count=len(df)
        )
        ...
        await self._publish_event(
            event_type,
            {"dataset_id": dataset_id, "summary": summary, "critical_issues": critical_errors > 0},
        )
                    ...
                    error_details = [
                        {
                            "row": int(idx),
                            "value": (
                                float(numeric_data.iloc[idx])
                                if pd.notna(numeric_data.iloc[idx])
                                else None
                            ),
                            "error": f"Out of range [{min_val}, {max_val}]",
                        }
                        for idx in range(len(error_mask))
                        if error_mask.iloc[idx]
                    ][:10]
            ...
            if not result.passed:
                if result.rule_name == "not_null_amount":
                    recommendations.append(
                        "Fill missing amount values or exclude incomplete records"
                    )
                elif result.rule_name == "positive_amount":
                    recommendations.append(
                        "Check data source for negative amounts - possible data error"
                    )
                    ...
                    recommendations.append(
                        f"High error rate ({result.error_rate:.1%}) in {result.field} - investigate data source"
                    )
