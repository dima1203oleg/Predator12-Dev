...
class CustomsCSVParser:
    ...
    def _get_numeric_columns(self) -> List[str]:
        ...
        return [
            ...
            "customs_value_usd_additional_unit",
            "customs_value_gross_usd_kg",
            "min_base_usd_kg",
            "min_base_diff",
            "net_customs_value_usd_kg",
            "customs_value_diff_usd_kg",
            "preferential_rate",
            "full_rate",
        ]
    ...
    def process_chunk(self, chunk_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "country_origin" in chunk_df.columns:
            chunk_df["country_origin"] = chunk_df["country_origin"].apply(
                self._normalize_country_code
            )

        if "country_dispatch" in chunk_df.columns:
            chunk_df["country_dispatch"] = chunk_df["country_dispatch"].apply(
                self._normalize_country_code
            )
