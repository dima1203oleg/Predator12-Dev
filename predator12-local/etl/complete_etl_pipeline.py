...
class CustomsETLPipeline:
    ...
    def detect_anomalies(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        price_per_kg = df["Фактурна варість, валюта контракту"] / df["Маса, нетто, кг"].replace(
            0, 1
        )
        ...
        mask3 = (
            (df["Країна відправлення"] != df["Країна походження"])
    ...
    def transform_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Parse dates
        transformed_df["declaration_date"] = transformed_df["Дата оформлення"].apply(
            self.parse_date
        )
        ...
        if self.config.enable_pii_masking:
            transformed_df["importer_edrpou_hash"] = transformed_df["importer_edrpou"].apply(
                self.hash_pii_field
            )
            transformed_df["importer_name_hash"] = transformed_df["importer_name"].apply(
                self.hash_pii_field
            )
            transformed_df["goods_description_hash"] = transformed_df["goods_description"].apply(
                self.hash_pii_field
            )
    ...
    def validate_data_quality(self, df: pd.DataFrame) -> Dict:
        ...
        total_issues = (
            results["missing_critical_fields"]
    ...
    def process_csv_file(self, file_path: str) -> Dict:
        ...
        try:
            ...
            if not quality_results["passed"]:
                error_msg = (
                    f"Data quality below threshold: {quality_results['data_quality_score']:.2%}"
                )
    ...
    def run_pipeline(self, input_files: List[str] = None) -> Dict:
        ...
        # Generate summary
        avg_quality_score = (
            sum(f["data_quality_score"] for f in pipeline_results["files_processed"])
            / len(pipeline_results["files_processed"])
            if pipeline_results["files_processed"]
            else 0
        )
        ...
        pipeline_results["summary"] = {
            ...
            "success_rate": (
                sum(1 for f in pipeline_results["files_processed"] if f["success"])
                / len(pipeline_results["files_processed"])
                if pipeline_results["files_processed"]
                else 0
            ),
            ...
            "anomaly_rate": (
                pipeline_results["total_anomalies"] / pipeline_results["total_rows_processed"]
                if pipeline_results["total_rows_processed"] > 0
                else 0
            ),
        }

        logger.info(
            f"Pipeline completed. Processed {pipeline_results['total_rows_processed']} rows in {pipeline_results['processing_time_seconds']:.2f} seconds"
        )
...
def main():
    ...
    if os.path.exists(csv_file):
        ...
        print(
            f"🚨 Anomalies detected: {results['summary']['total_anomalies']:,} ({results['summary']['anomaly_rate']:.1%})"
        )
