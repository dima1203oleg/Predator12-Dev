    ...
    for file in os.listdir(PARQUET_DIR):
        if file.endswith(".parquet"):
            ...
            if "Дата оформлення" in df.columns:
                df["Дата оформлення"] = pd.to_datetime(
                    df["Дата оформлення"], errors="coerce"
                ).dt.strftime("%Y-%m-%d")
...
def load():
    ...
    try:
        ...
        for file in os.listdir(PARQUET_DIR):
            if file.endswith(".parquet"):
                pd.read_parquet(os.path.join(PARQUET_DIR, file)).to_csv(
                    os.path.join(BACKUP_DIR, f"{file}.csv"), index=False
                )
        ...
        for file in os.listdir(PARQUET_DIR):
            if file.endswith(".parquet"):
                ...
                for _, row in df.head(1000).iterrows():
                    actions.append({"_index": index, "_source": row.dropna().to_dict()})
        if actions:
...
with DAG(
    "import_declarations", schedule_interval=None, catchup=False, default_args=default_args
) as dag:
