...
class ModelSDK:
    ...
    async def _call_api_provider(
        self, provider: str, endpoint: str, request: ChatRequest
    ) -> ChatResponse:
    ...
    async def _fallback_response(self, request: ChatRequest) -> ChatResponse:
        ...
        return ChatResponse(
            ...
            usage={
                ...
                "total_tokens": (
                    len(last.split()) + len(content.split()) if last else len(content.split())
                ),
            },
        )
...
async def detailed_status():
    ...
    return {
        ...
        "top_models": [
            m["id"] for m in sorted(models, key=lambda x: x["quality_score"], reverse=True)[:10]
        ],
    }
...
if __name__ == "__main__":
    ...
    uvicorn.run("model_server:app", host="0.0.0.0", port=port, log_level="info", reload=False)
