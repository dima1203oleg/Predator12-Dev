...
- [5. Docker Deployment](#5-docker-deployment) - [Running Docker Containers](#running-docker-containers)
...
- **Configuration**: For Claude Desktop, add to `claude_desktop_config.json`:
  ```json
  {
    "mcpServers": {
      "okta-mcp-server": {
        ...
        "args": ["DIR/okta-mcp-server/main.py"],
        "env": {
  ...
  ```
  _Replace `DIR` with your absolute directory path and `OKTA_API_TOKEN` with your actual token_
