...
async def chat_completions(request: ChatRequest):
    ...
    response_text = (
        f"Відповідь від моделі {request.model}: {request.messages[-1]['content'][:100]}..."
    )
    ...
    return ChatResponse(
        ...
        usage={
            ...
            "total_tokens": (
                sum(len(msg["content"]) for msg in request.messages) + len(response_text)
            )
            // 4,
        },
    )
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "standalone_model_server:app", host="0.0.0.0", port=3010, reload=False, log_level="info"
    )
